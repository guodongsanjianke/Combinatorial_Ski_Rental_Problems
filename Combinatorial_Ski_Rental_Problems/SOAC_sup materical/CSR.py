...
def csr(b, r, discount=None, fixed_partitions=None, b_dict=None, opt_lines=None, eval_T=None):
    ...
    if eval_T is not None:
        extend_T = max(int(np.max(2 * np.array(b) / np.array(r))),eval_T)
    else:
        eval_T = extend_T = int(np.max(2 * np.array(b) / np.array(r)))
    ...
    for i in range(len(p_list)):
        ...
        p_list_dict[partitions_list[i].p_id][partitions_list[i].split] = basic_func.p_list_extend(p_list[i], extend_T,
                                                                                                  start_day[i] - 1)
...
def evaluate(all_path, opt_result, eval_T,num_evaluate=10000):
    ...
    for end_T in trange(1, eval_T):
        # end_T = random.randint(1, eval_T - 1)
        temp = []
